...
| `padding`   | string  | `all medium`    | `all`, `top`, `right`, `bottom`, `left`, user-defined<sup>1</sup>               | The padding property of the div generated. The first word is the position and the second is the size of the padding. Can have up to 4 paddings separated by spaces. Sizes come from user-defined variable names set/overwritten in variables.scss. Must be a space in between the position and the size. |
...
<sup>1</sup> extras is not a prop exactly, you dont give `extras = { blah }`, but you simply give the extra properties themselves and it gets captured and passed on directly to the Container. For example, putting `extras = {{ direction: 'row' }}` will not work, but putting `direction = 'row'` will pass the direction property to the Container. extras just means any other arguments passed. See examples for more.
