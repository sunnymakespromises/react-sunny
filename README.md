...
import { Container, Dropdown, Header, List, Option, Text } from 'react-sunny-components'
...
const DropdownExample = () => {
    ...
    return (
        <Dropdown onToggle = { (v) => toggle(v) } onSelect = { (v) => select(v) } behavior = 'click' trigger = 'header' container>
...
import { Container, Responsive, Text } from 'react-sunny-components'
...
| `fill`      | string  | `parent`        | `width`, `height`, `parent`, `screen`, `auto`, `none`                | The way in which the div generated wants to fill space.                                                                                                                                                                                                                                                  |
| `padding`   | string  | `all medium`    | `all`, `top`, `right`, `bottom`, `left`, user-defined*               | The padding property of the div generated. The first word is the position and the second is the size of the padding. Can have up to 4 paddings separated by spaces. Sizes come from user-defined variable names set/overwritten in variables.scss. Must be a space in between the position and the size. |
| `margin`    | string  | `all none`      | `all`, `top`, `right`, `bottom`, `left`, user-defined*               | The margin property of the div generated. The first word is the position and the second is the size of the margin. Can have up to 4 margins separated by spaces. Sizes come from user-defined variable names set/overwritten in variables.scss. Must be a space in between the position and the size.    |
| `color`     | string  | empty string    | user-defined*                                                        | The background-color property of the div generated. Will take a hex code or a user-defined variable name set/overwritten in variables.scss.                                                                                                                                                              |
...
| `extras`**  | any     | none            | none                                                                 | Any other properties to be passed directly to the generated div, i.e. events or anything else I forgot.                                                                                                                                                                                                  |
| `ref`       | ref     | none            | none                                                                 | A React ref to be passed to the div.                                                                                                                                                                                                                                                                     |
* See [User-Defined Variables](#user-defined-variables).
** extras is not a prop exactly, you dont give `extras = { blah }`, but you simply give the extra properties themselves and it gets captured and passed on directly to the div component. For example, putting `extras = {{ onClick: () => click('test') }}` will not work, but putting `onClick = { () => click('text') }` will pass the onClick property to the div. extras just means any other arguments passed. See example for more.
...
| `onSelect`  | function | null    | none               | The function that gets called when the dropdown option gets clicked.                                                                                                                                                                                                                                                                                                                                                                  |
...
| `extras`*   | any      | none    | none               | Any property that you want passed through to a generated wrapper [Container](#container). If any property like `direction`, or `alignment`, etc is given, a [Container](#container) with those properties will be generated as a wrapper. If none is given, then the component will simply render its children, unless `container` is true, in which case it will generate a default [Container](#container) with just default props. |

* extras is not a prop exactly, you dont give `extras = { blah }`, but you simply give the extra properties themselves and it gets captured and passed on directly to the Container. For example, putting `extras = {{ direction: 'row' }}` will not work, but putting `direction = 'row'` will pass the direction property to the Container. extras just means any other arguments passed. See examples for more.

##### Option
Option is the only Dropdown subcomponent that has a prop other than `container` and `extras`, as it also takes in a `value` prop. Header, List, and Button all only have `container` and `extras` props, which both have the same behavior as in the Dropdown component.
...
```
<Dropdown onToggle = { (v) => toggle(v) } onSelect = { (v) => select(v) } behavior = 'click' trigger = 'button' fill = 'parent'>
...
| `color`    | string | empty string | user-defined*               | The color property of the generated text.                                                                |
| `size`     | string | `h1`         | `h1`, `h2`, `h3`, `h4`, `p` | The size of the generated text.                                                                          |
| `style`    | string | `normal`     | `normal`, `italic`, `bold`  | The font style of the generated text.                                                                    |
| `extras`** | any    | none         | none                        | Any other properties to be passed directly to the generated text, i.e. events or anything else I forgot. |

##### Example
```
<Text size = 'h1' style = 'bold'>
...
<Text size = 'h3'>
    This text is a pretty good size.
</Text>
...
<>
    <Responsive portrait>
        <Text>
            This gets shown when the device is portrait.
        </Text>
    </Responsive>

    <Responsive landscape>
        <Text>
            This gets shown when the device is landscape.
        </Text>
    </Responsive>
</>
```
